...
AccountOpResult Battlenet::AccountMgr::CreateBattlenetAccount(std::string email, std::string password)
{
    ...
    uint32 bnetAccountId = NULL;
    do{
    bnetAccountId = Battlenet::AccountMgr::GetId(email);
    }while (bnetAccountId == NULL);

    PreparedStatement* stmt2 = LoginDatabase.GetPreparedStatement(LOGIN_INS_ACCOUNT);
    stmt2->setString(0, email);
    stmt2->setString(1, CalculateShaPassHash(email, password));
    stmt2->setUInt32(2, bnetAccountId);
...
void Battlenet::AccountMgr::GetVoid()
{
}
